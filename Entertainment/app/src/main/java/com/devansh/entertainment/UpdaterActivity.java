...
public class UpdaterActivity extends AppCompatActivity {
    ...
    public void downloadNow(View v)
    {
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O) {
            if (!getPackageManager().canRequestPackageInstalls()) {
                Toast.makeText(getApplicationContext(), "Allow "+getString(R.string.app_name)+" to install apk", Toast.LENGTH_SHORT).show();
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        startActivity(new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES, Uri.parse("package:com.devansh.entertainment")));
                    }
                }, 1000);
                return;
            }
        ...
        request.setDestinationInExternalFilesDir(getApplicationContext(), "",
                getString(R.string.app_name)+"_" + app_update.replace(' ', '_') +
                        ".apk");
            ...
            public void run() {
                ...
                while (downloading) {
                    ...
                    try {
                        ...
                        cursor.moveToFirst();final int bytes_downloaded = cursor.getInt(cursor.getColumnIndex(DownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR));
    ...
    public void onBackPressed() {
        super.onBackPressed();
        File file = new File(getApplicationContext().getExternalFilesDir(null),getString(R.string.app_name)+"_"+app_update.replace(' ','_')+".apk");
